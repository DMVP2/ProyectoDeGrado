...
$usuario = $manejoUsuario->buscarUsuario($codigo);

try {
    if(strcasecmp($usuario->getStatus(), "Activo") == 0)
    {
        $manejoUsuario->desactivarUsuario($codigo);
    }
    else if(strcasecmp($usuario->getStatus(), "Inactivo") == 0)
    {
        $manejoUsuario->activarUsuario($codigo);
    }
    echo "<script>
